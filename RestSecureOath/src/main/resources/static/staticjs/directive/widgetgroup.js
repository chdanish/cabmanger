'use strict'
app.directive('widgetGroup',['$compile','$timeout','cacheService', function($compile,$timeout,cacheService) {
	
	var group = /*'<div class="container col-xs-2 seperation">'+
...
'</div>'*/ ;
  ...
  return {
    ...
    link: function(scope, element, attrs) {
    	 scope.widgettitle="Group";
    	 element.append(group);
         $compile(element.contents())(scope); 
         cacheService.update('/a_miniwidget/group').then(function(data){
      		console.log(data);
      		scope.total=data.total[0].groups-1;// subtracting owner from employee count
